...
class Sintel(BaseStereoDataset):
    """MPI Sintel dataset for training. 

    Args:
        pass_style (str): Pass style for Sintel dataset, and it has 2
    ...
    def _get_data_dir(self) -> None:
        ...
        pass_dir = self.pass_style
        img1s_dirs = []
        img2s_dirs = []
        disp_dirs = []

        img1s_dirs_ = glob(
            osp.join(self.data_root, 'training/' + pass_dir + '_left/*/'))
        img1s_dirs += img1s_dirs_

        img2s_dirs_ = glob(
            osp.join(self.data_root, 'training/' + pass_dir + '_right/*/'))
        img2s_dirs += img2s_dirs_

        disp_dirs_ = glob(
            osp.join(self.data_root, 'training/' + 'disparities' + '/*/'))
