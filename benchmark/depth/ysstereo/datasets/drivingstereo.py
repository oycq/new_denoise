...
class DrivingStereo(BaseStereoDataset):
    """DrivingStereo dataset for training. 
    ...
    def _get_data_dir(self) -> None:
        ...
        img1s_dirs_ = glob(
            osp.join(self.data_root, 'train/left/*'))
        img1s_dirs += img1s_dirs_

        img2s_dirs_ = glob(
            osp.join(self.data_root, 'train/right/*'))
        img2s_dirs += img2s_dirs_

        if self.sky:
            disp_dirs_ = glob(
                osp.join(self.data_root, 'train/disparity_sky/*'))
        else:
            disp_dirs_ = glob(
                osp.join(self.data_root, 'train/disparity/*'))
