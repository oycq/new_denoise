...
class FisheyeSimOutdoor(BaseStereoDataset):
    """FisheyeSim subset dataset.
    """
    ...
    def load_data_info(self) -> None:
        ...
        for i in range(len(img1_filenames)-1, -1, -1):
            idx = int(img1_filenames[i].split('/')[-1].split('.')[0])
            if idx < 19302:
                del img1_filenames[i], img2_filenames[i], disp_filenames[i]
    ...
    def _get_data_dir(self) -> None:
        ...
        disp_dirs_ = glob(
            osp.join(self.data_root, 'radial_disp/'))
