...
class LoadDispAnnotations(LoadAnnotations):
    ...
    def __init__(
            ...
            dev3_lidar_gt: bool = False,
            dilate_disp: bool = False,
            uemrq_mode: bool = False,
            dilate_kernel: int = 3,
    ...
    def _load_disp(self, results: dict) -> dict:
        ...
        for filename in filenames:
            ...
            if filename.find('disp') > -1:
                ...
                if self.sintel_mode:
                ...
                elif self.uemrq_mode:
                    # is pfm
                    depth = read_pfm(filename_disp) # depth in meters
                    baseline = 0.05
                    focal_length = (1.0 / np.tan(np.radians(90.0) / 2.0)) * (1280.0 / 2.0)
                    ...
                    if "defom_pgt" in filename_disp:
                        if self.dev3_lidar_gt:
                            # load disparity
                            ss = "defom_pgt2" if "defom_pgt2" in filename_disp else "defom_pgt"
                            load_name = filename_disp.replace(ss, "depth_0").replace("_left.pfm", ".pfm")
                            lidar_depth = read_pfm(load_name)
...
class LoadPseudoAnnotations(LoadAnnotations):
    ...
    def __init__(
            ...
            dev3_mode: bool = False,
            dev3_lidar_mode: bool = False,
            load_right_psd_disp: bool = False,
            dan_v: str = "v1",
    ) -> None:
        ...
        self.dev3_mode = dev3_mode
        self.dev3_lidar_mode = dev3_lidar_mode
        self.load_right_psd_disp = load_right_psd_disp
        self.dan_v = dan_v
        ...
        for filename in filenames:
            ...
            if filename.find('disp') > -1:
                ...
                if self.davanet_mode:
                    ...
                    if "defom_pgt" in filename_disp:
                        ss = "defom_pgt2" if "defom_pgt2" in filename_disp else "defom_pgt"
                        depth_anything_file = filename_disp.replace(ss, "left_depth_anything_v1").replace("_left.pfm", ".exr")
                    else:
                        depth_anything_file = filename_disp
                    if self.dan_v=="v2":
                        depth_anything_file = filename_disp.replace("v1", "v2")
                    else:
                        depth_anything_file = filename_disp
                    pseudo_disp = cv2.imread(depth_anything_file, cv2.IMREAD_ANYCOLOR | cv2.IMREAD_ANYDEPTH)
                    if len(pseudo_disp.shape) > 2:
                        pseudo_disp = pseudo_disp[..., 0]
                    valid = np.ones_like(pseudo_disp[:1, :1])
                    if self.load_right_psd_disp:
                        right_psd_disp = cv2.imread(depth_anything_file.replace("left", "right"), cv2.IMREAD_ANYCOLOR | cv2.IMREAD_ANYDEPTH)
                        if len(pseudo_disp.shape) > 2:
                            pseudo_disp = pseudo_disp[..., 0]
                elif self.dev3_mode:
                    if "defom_pgt" in filename_disp:
                        ss = "defom_pgt2" if "defom_pgt2" in filename_disp else "defom_pgt"
                        depth_anything_file = filename_disp.replace(ss, "left_depth_anything_v2").replace("_left.pfm", ".exr")
                    else:
                        depth_anything_file = filename_disp
                    pseudo_disp = cv2.imread(depth_anything_file, cv2.IMREAD_ANYCOLOR | cv2.IMREAD_ANYDEPTH)
                    if len(pseudo_disp.shape) > 2:
                    ...
                    valid = np.ones_like(pseudo_disp[:1, :1])
                    if self.load_right_psd_disp:
                        right_psd_disp = cv2.imread(filename_disp.replace("left", "right"), cv2.IMREAD_ANYCOLOR | cv2.IMREAD_ANYDEPTH)
                        if len(pseudo_disp.shape) > 2:
                            pseudo_disp = pseudo_disp[..., 0]
                elif self.dev3_lidar_mode:
                    if "defom_pgt" in filename_disp:
                        ss = "defom_pgt2" if "defom_pgt2" in filename_disp else "defom_pgt"
                        depth_anything_file = filename_disp.replace(ss, "DAdepth_0").replace("_left.pfm", ".pfm")
                    else:
                        depth_anything_file = filename_disp
                    pseudo_disp = cv2.imread(depth_anything_file, cv2.IMREAD_ANYCOLOR | cv2.IMREAD_ANYDEPTH)
                    if len(pseudo_disp.shape) > 2:
                ...
                pseudo_disp = (pseudo_disp-median)/scele
                
                if self.load_right_psd_disp:
                    if right_psd_disp is not None:
                        median = np.median(right_psd_disp)
                        scele = np.mean(np.abs(right_psd_disp-median))
                        right_psd_disp = (right_psd_disp-median)/scele
                        right_valid = np.ones_like(right_psd_disp[:1, :1])
                    else:
                        right_psd_disp = pseudo_disp
                        right_valid = np.zeros_like(right_psd_disp[:1, :1])
                    ann_key = "right_pseudo_gt"
                    results[ann_key] = right_psd_disp
                    results["right_pseudo_valid"] = right_valid
                    results['ann_fields'].append(ann_key)
