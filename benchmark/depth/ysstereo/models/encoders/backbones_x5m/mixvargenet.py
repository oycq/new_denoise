...
from ysstereo.models.encoders.backbones_x5m.basic_mixvargenet_module import MixVarGEBlock
from ysstereo.models.encoders.backbones_x5m.conv_module import ConvModule2d
from ysstereo.models.encoders.backbones_x5m.extend_container import ExtSequential
...
GroupInNetConfig = dict(
    net_config = [
            [
                MixVarGENetConfig(
                    in_channels=32,
                    out_channels=32,
                    head_op="mixvarge_f1_gb16",
                    stack_ops=[],
                    stride=1,
                    stack_factor=1,
                    fusion_strides=[],
            ...
            [
                MixVarGENetConfig(
                    ...
                    out_channels=32,
                    head_op="mixvarge_f2_gb16",
                    stack_ops=["mixvarge_f4_gb16", "mixvarge_f2_gb16"],
                    stride=2,
                    stack_factor=1,
                    fusion_strides=[],
                    extra_downsample_num=0,
...
MIXVARGENet_ARCH_CFGS = dict(
    base=BaseNetConfig, quat=QuatorNetConfig, half=HalfNetConfig,
    less_stack=LessStackNetConfig, groupin = GroupInNetConfig,
    light_in=LightInNetConfig, tuned=TunedNetConfig,
)
...
class MixVarGENet(nn.Module):
    ...
    def process_sequence_input(self, x: List) -> torch.Tensor:
