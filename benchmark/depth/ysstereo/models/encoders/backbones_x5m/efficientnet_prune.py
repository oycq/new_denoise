...
from ysstereo.models.encoders.backbones_x5m.basic_efficientnet_module import MBConvBlock_prune
...
DEFAULT_BLOCKS_ARGS_OPT2 = (
    ...
    BlockArgs(
        ...
        num_repeat=4,
        in_filters=56,
        out_filters=80, #96 else 80 opt2
        expand_ratio=6,
        id_skip=True,
        strides=2,
        se_ratio=0.25,
...
class EfficientNet(nn.Module):
    ...
    def __init__(
        ...
        if model_type == "lite0tfatopt2":
            blocks_args = DEFAULT_BLOCKS_ARGS_OPT2
        elif model_type == "lite0tfatopt4":
            blocks_args = DEFAULT_BLOCKS_ARGS_OPT4
        ...
        if self.use_lite:
            if self.width_coefficient in [1.0, 1.2] :
                out_planes = prune_cfg[0]
            else:
                out_planes = 32
        else:
            out_planes = round_filters(
                48, self.width_coefficient, self.depth_division
            )
        ...
        for idx, block_args in enumerate(self.blocks_args):
            ...
            self._blocks_conv.append(
                MBConvBlock_prune(
                    ...
                    act_layer=copy.deepcopy(act_layer),
                    prune_channels = self.prune_cfg.pop(0),
...
def _efficient(mode_type, coefficient_params, prune_cfg, **kwargs):
    efficient = functools.partial(EfficientNet, mode_type, coefficient_params, prune_cfg)
    return efficient(**kwargs)


def efficientnet(model_type, prune_cfg, **kwargs):  # noqa: D200, D401
    """
    A module of efficientnet.
...
def efficientnet_lite_prune(model_type, prune_cfg, **kwargs):  # noqa: D200, D401
    """
    ...
    coefficient_params_dict = {
        ...
        "lite0tfatopt2":(1.0, 0.5, 224, 0.2),
        "lite0tfatopt4":(1.0, 0.5, 224, 0.2),
        "lite0thuge":(1.2, 0.5, 224, 0.2),
    ...
    if "coefficient_params" in kwargs:
        ...
        coefficient_params = coefficient_params_dict[model_type]
    return _efficient(model_type, coefficient_params, prune_cfg, **kwargs)
