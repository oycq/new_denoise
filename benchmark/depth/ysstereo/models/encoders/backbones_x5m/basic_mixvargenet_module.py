...
from ysstereo.models.encoders.backbones_x5m.reparam import OnlineReParamBlock
...
class BasicMixVarGEBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.fusion_layers is not None:
            ...
            for _idx, (fusion_input, fusion_layer) in enumerate(
                zip(fusion_inputs, self.fusion_layers)
            ):
                fusion_ouput = fusion_layer(fusion_input)
                out = fusion_ouput + out
