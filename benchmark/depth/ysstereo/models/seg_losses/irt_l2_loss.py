...
class VideoIrtL2Loss(nn.Module):
    """L2 loss to improve the temporal consistency.
    ...
    def __init__(self, loss_name='loss_irt_l2', coeff=1.0):
        super().__init__()
    ...
    def forward(self, data, it, **kwargs):
        ...
        for i in range(1, s):
            ...
            for j in range(b):
                ...
                prediction_main = prediction[:, :1].float()
                prediction_minor = prediction[:, 1:].float()
                net_gt = data['alphas'][j:j + 1, i].float()
                net_in = net_gt

                loss_l1 = F.mse_loss(prediction_main, net_gt)
                ...
                confidence_map_l = torch.lt(diff_map_main,
                                            diff_map_minor).float()
                loss_major = F.mse_loss(prediction_main * confidence_map_l,
                                        net_gt * confidence_map_l)
                loss_minor = F.mse_loss(
                    prediction_minor * (1 - confidence_map),
