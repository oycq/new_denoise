...
from ysstereo.models.depth_decoders.decoder_submodules import CorrBlock1D, MotionEncoder, ConvGRU, XHead
...
class HresStereoDecoder(BaseDecoder):
    """The decoder of HresStereo Net.

    The decoder of HresStereo Net, which outputs list of upsampled stereo estimation.
    ...
    Args:
        ...
        act_cfg (dict, optional): Config dict of activation layer in motion
            encoder. Default: None.
    """
    _h_channels = {'Basic': 16, 'Small': 96}
    _cxt_channels = {'Basic': 16, 'Small': 64}
    ...
    def __init__(
        ...
        self.corr_block = CorrBlock1D(num_levels=num_levels)
    ...
    def make_gru08_block(self, num_layers):
        return ConvGRU(
            self.h_channels,
            self.encoder.out_channels[0] + 2 + self.cxt_channels * (num_layers > 1),
            net_type=self.gru_type)

    def make_gru16_block(self, num_layers):
        return ConvGRU(
            self.h_channels,
            self.encoder.out_channels[0] + 2 + self.cxt_channels * (num_layers == 3),
            net_type=self.gru_type)

    def make_gru32_block(self, num_layers):
        return ConvGRU(
            self.h_channels,
            self.encoder.out_channels[0] + 2,
            net_type=self.gru_type)
    ...
    def initialize_flow(self, flow):
        ...
        coords0 = coords_grid(N, H, W).to(flow.device)
        coords1 = coords_grid(N, H, W).to(flow.device)
    ...
    def forward(self, feat1: torch.Tensor, feat2: torch.Tensor,
                flow_init: torch.Tensor, net_list: Sequence[torch.Tensor],
                inp_list: Sequence[torch.Tensor]) -> Sequence[torch.Tensor]:
        ...
        for _ in range(self.iters):
            ...
            corr = self.corr_lookup(corr_pyramid, coords1)
            ...
            upflow = self._upsample(coords1 - coords0, mask)
            upflow = upflow[:, :1]
    ...
    def forward_train(
        ...
        flow_pred = self.forward(feat1, feat2, flow, net_list, inp_list)
