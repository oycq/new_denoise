...
class RumStereoDecoder(BaseDecoder):
    ...
    def __init__(
        ...
        # adaptive search
        self.search_num = 9
        self.update_block = BasicUpdateBlock(radius, gru_type, conv_type,
    ...
    def forward(self, fmap1, fmap2, flow_init=None, test_mode=False):
        ...
        if flow_init is not None:
            ...
            # zero initialization
            flow_dw8 = self.zero_init(fmap1_dw8)
            ...
            for itr in range(5):
                ...
                out_corrs = corr_iter(fmap1_dw8, fmap2_dw8, flow_dw8, psize, dilate)

                net_dw8, up_mask, delta_flow = self.update_block(
                    net_dw8, inp_dw8, out_corrs, flow_dw8, output_all=True)

                flow_dw8 = flow_dw8 + delta_flow
                flow = convex_upsample(flow_dw8, up_mask, rate=8)
        ...
        for itr in range(self.iters):
            flow = flow.detach()
            out_corrs = corr_iter(fmap1, fmap2, flow, psize, dilate)

            net, up_mask, delta_flow = self.update_block(net, inp, out_corrs, flow, output_all=True)
    ...
    def forward_train(self,
        ...
        if return_preds:
            return self.losses(disp_pred, disp_gt, valid=valid), disp_pred
        else:
    ...
    def forward_onnx_gridsample(self, fmap1: torch.Tensor):
        coords0 = coords_grid(fmap1.shape[0], fmap1.shape[2], fmap1.shape[3], fmap1.device)
        coords1 = coords0
        _, _, H, W = fmap1.shape
    ...
    def forward_onnx_rumstereo(self,
        ...
        dw8_coords0 = coords_grid(fmap1.shape[0], fmap1.shape[2], fmap1.shape[3], fmap1.device)
        for itr in range(self.iters):
