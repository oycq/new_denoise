...
class BasicUpdateBlock(nn.Module):
    ...
    def get_ctx_list(self, inp):
        return list(self.context_zqr_convs(inp).split(split_size=self.h_channels, dim=1))
    ...
    def forward(self, net, ctx_list, corr=None, disp=None, mask_flag=True, output_all=False):
        cz, cr, cq = ctx_list

        if output_all:
            motion_features = self.encoder(corr, disp)

            net = self.gru(net, cz, cr, cq, motion_features)
...
class NewRumStereoSlimDecoder(BaseDecoder):
    ...
    def __init__(
        ...
        self.update_block_8 = BasicUpdateBlock(radius, gru_type, conv_type,
                                               ...
                                               use_hard_tanh, use_hard_sigmoid, conv_cfg, norm_cfg,
                                               act_cfg)
        ...
        if self.shared_components is not None:
            ...
            if 'gru' in self.shared_components:
                del self.update_block_8.context_zqr_convs
                self.update_block_8.context_zqr_convs = self.update_block_16.context_zqr_convs
                del self.update_block_8.gru
                self.update_block_8.gru = self.update_block_16.gru
            if 'head' in self.shared_components:
                del self.update_block_8.disp_pred
                del self.update_block_8.mask_pred
                self.update_block_8.disp_pred = self.update_block_16.disp_pred
    ...
    def forward(self, fmaps1, fmaps2, disp_init=None, test_mode=False):
        # feature
        fmap1_16 = fmaps1[0]
        fmap2_16 = fmaps2[0]
        ...
        net_16 = self.tanh(net_16)
        inp_16 = F.relu(inp_16)
        ctx_list_16 = self.update_block_16.get_ctx_list(inp_16)
        
        net_8, inp_8 = torch.split(fmap1_8, [self.h_channels, self.h_channels], dim=1)
        net_8 = self.tanh(net_8)
        inp_8 = F.relu(inp_8)
        ctx_list_8 = self.update_block_8.get_ctx_list(inp_8)
        ...
        if disp_init is not None:
            ...
            # zero initialization
            disp_16 = self.zero_init(fmap1_16)
            ...
            for itr in range(self.iters_d16):
                ...
                net_16, up_mask, delta_disp = self.update_block_16(
                    net_16, ctx_list_16, out_corrs, disp_16, output_all=True)
        ...
        for itr in range(self.iters):
            ...
            net_8, up_mask, delta_disp = self.update_block_8(net_8, ctx_list_8, out_corrs, disp, output_all=True)
    ...
    def forward_train(self,
        ...
        if return_preds:
            return self.losses(disp_pred, **data_samples), disp_pred
        else:
            return self.losses(disp_pred, **data_samples)
    ...
    def forward_test(self,
                     ...
                     **data_samples) -> Sequence[Dict[str, np.ndarray]]:
        """Forward function when model training.
        ...
        fmaps1_1 = list(fmaps1[1])
        fmaps2_1 = list(fmaps2[1])
        disp_result = [dict(disp=disp_result[i], fmaps0=fmaps1_1[i], fmaps1=fmaps2_1[i],) for i in range(len(disp_result))]

        return self.get_disp(disp_result, **data_samples)
