...
def avg_max_reduce_channel(x):
    """
    Applies average and max pooling along the channel dimension (axis=1),
    supports single tensor or list/tuple of tensors.
    
    Returns:
        A tensor with concatenated mean and max results from each input.
    """
    def _reduce(tensor):
    ...
    if isinstance(x, (list, tuple)):
        results = []
        for xi in x:
            mean, max_val = _reduce(xi)
            results.extend([mean, max_val])
        return torch.cat(results, dim=1)
    else:
        mean, max_val = _reduce(x)
        return torch.cat([mean, max_val], dim=1)

class UAFM(nn.Module):
    """
    Base Unified Attention Fusion Module (Paddle → PyTorch).
    """
    def __init__(self, x_ch, y_ch, out_ch, ksize=3, resize_mode='bilinear'):
        super().__init__()
        # conv_x: x_ch → y_ch
        self.conv_x = ConvModule(
            x_ch, y_ch, kernel_size=ksize, stride=1, padding=ksize//2,
            norm_cfg=dict(type='BN'), act_cfg=dict(type='ReLU'), bias=False)
        # conv_out: y_ch → out_ch
        self.conv_out = ConvModule(
            y_ch, out_ch, kernel_size=3, stride=1, padding=1,
            norm_cfg=dict(type='BN'), act_cfg=dict(type='ReLU'), bias=False)
        self.resize_mode = resize_mode

    def forward(self, x: torch.Tensor, y: torch.Tensor) -> torch.Tensor:
        # x: low-level [B, x_ch, H1, W1]
        # y: high-level [B, y_ch, H2, W2] with H2<=H1, W2<=W1
        # 1) check shapes
        assert x.dim()==4 and y.dim()==4
        assert x.shape[2]>=y.shape[2] and x.shape[3]>=y.shape[3]
        # 2) prepare
        x_p = self.conv_x(x)
        y_p = F.interpolate(y, size=x_p.shape[2:], mode=self.resize_mode, align_corners=False)
        # 3) fuse
        out = x_p + y_p
...
class UAFM_SpAtten(UAFM):
    """
    UAFM with spatial attention (Paddle → PyTorch port).
    """
    def __init__(self, x_ch, y_ch, out_ch, ksize=3, resize_mode='bilinear'):
        super().__init__(x_ch, y_ch, out_ch, ksize, resize_mode)
        # spatial-attention head: 4→2→1 channels
        self.conv_xy_atten = nn.Sequential(
            ConvModule(4, 2, kernel_size=3, padding=1,
                       norm_cfg=dict(type='BN'), act_cfg=dict(type='ReLU'),
                       bias=False),
            ConvModule(2, 1, kernel_size=3, padding=1,
                       norm_cfg=dict(type='BN'), act_cfg=None,
                       bias=False)
        )
        # scale parameter, fixed to 1.0
        self.scale = nn.Parameter(torch.tensor(1.0), requires_grad=False)

    def forward(self, x, y):
        # reuse conv_x and conv_out from base, but override fuse logic
        x_p = self.conv_x(x)
        y_u = F.interpolate(y, size=x_p.shape[2:], mode=self.resize_mode, align_corners=False)
        # compute spatial attention on concat avg+max over [x_p, y_u]
        atten = avg_max_reduce_channel([x_p, y_u])      # [B,4,H,W]
        atten = torch.sigmoid(self.conv_xy_atten(atten))# [B,1,H,W]
        # gated fusion: x * A + y * (scale - A)
        fused = x_p * atten + y_u * (self.scale - atten)
        # final conv_out
...
class UAFMMobile_SpAtten(UAFM):
    """
    Unified Attention Fusion Module + Spatial Attention for mobile.
    """
    def __init__(self, x_ch, y_ch, out_ch, ksize=3, resize_mode='bilinear'):
        super().__init__(x_ch, y_ch, out_ch, ksize, resize_mode)
        # override conv_x and conv_out with separable convs:
        # depthwise
        self.conv_x = ConvModule(
            x_ch, x_ch, kernel_size=ksize, stride=1, padding=ksize//2,
            groups=x_ch, norm_cfg=dict(type='BN'), act_cfg=dict(type='ReLU'), bias=False)
        # pointwise
        self.conv_x.add_module('pw', nn.Conv2d(x_ch, y_ch, 1, bias=False))
        self.conv_x.add_module('pw_bn', nn.BatchNorm2d(y_ch))
        self.conv_x.add_module('pw_relu', nn.ReLU(inplace=True))

        self.conv_out = ConvModule(
            y_ch, y_ch, kernel_size=3, stride=1, padding=1,
            groups=y_ch, norm_cfg=dict(type='BN'), act_cfg=dict(type='ReLU'), bias=False)
        self.conv_out.add_module('pw', nn.Conv2d(y_ch, out_ch, 1, bias=False))
        self.conv_out.add_module('pw_bn', nn.BatchNorm2d(out_ch))
        self.conv_out.add_module('pw_relu', nn.ReLU(inplace=True))

        # spatial attention head: 4→2→1 channels
        self.conv_xy_atten = nn.Sequential(
            ConvModule(2, 2, kernel_size=3, stride=1, padding=1,
                       norm_cfg=dict(type='BN'), act_cfg=dict(type='ReLU'), bias=False),
    ...
    def fuse(self, x: torch.Tensor, y: torch.Tensor) -> torch.Tensor:
        # compute spatial attention over concatenated avg+max of x and y
        # first, upsample y
        y_up = F.interpolate(y, size=x.shape[2:], mode=self.resize_mode, align_corners=False)
        # x and y already prepared by base .forward; but here fuse overrides
        # spatial attention:
        cat = torch.cat(avg_max_reduce_channel(x * 1.0), dim=1)  # [B,4,H,W]
        atten = torch.sigmoid(self.conv_xy_atten(cat))           # [B,1,H,W]
        # gated fusion
        U = x * atten + y_up * (1 - atten)
        out = self.conv_out(U)
        return out

    def forward(self, x, y):
        # reuse base check & prepare
        assert x.dim()==4 and y.dim()==4
        x_p = self.conv_x(x)
        y_up = F.interpolate(y, size=x_p.shape[2:], mode=self.resize_mode, align_corners=False)
        return self.fuse(x_p, y_up)
...
class SPASPP(nn.Module):
    def __init__(self, in_planes, inter_planes, out_planes, block_num=3, stride=1, dilation=[6,12,18]): # dilation=[6,12,18,24]
        ...
        self.process2 = nn.Sequential(
            ...
            nn.Conv2d(inter_planes*4, out_planes, kernel_size=1, padding=0, bias=False ),
            BatchNorm2d(out_planes, momentum=bn_mom),
            nn.ReLU(inplace=True)
        )
