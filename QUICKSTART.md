...
**Q：想换 lam 或换损失重新扫描**
A：本仓库不再追踪测试脚本（已 gitignore）。从原仓库的本地 backup 找
`run_experiments.py` / `evaluate_experiments.py` / `multi_scene_infer.py` /
`generate_20rois.py` 即可。
或者直接改 `n2n/trainer.py` 里的 `loss_kwargs={"lam": 你想要的}`，
跑一次 `python run_train.py` 见效。
